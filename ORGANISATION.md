# Organisation de fireviewer-geolocation

Source canonique : **[fireviewer/fireviewer-geolocation](https://github.com/fireviewer/fireviewer-geolocation)**. Responsabilité technique : **FV**. Accès : **private**.

Hypothèses géographiques, Panoramax, retrieval et registration.

## Où travailler et commiter

Dans le workspace organisé, ouvrir **`actifs/fireviewer/fireviewer-geolocation`**. Chaque dossier est un dépôt Git autonome. Créer une branche de chantier dans ce dépôt ; pour un travail parallèle, créer un worktree sous `travaux/<chantier>/<composant>`. Ne jamais commiter depuis la racine multi-dépôts, `archives/` ou un ancien chemin de compatibilité.

Estimation spatiale avec référentiel, provenance et incertitude.

Une hypothèse ne devient pas une position publique validée sur simple fin du calcul.
